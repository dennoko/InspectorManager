...
複数のオブジェクトをまとめて選択した場合は、Unity標準と同じマルチ編集表示になります。
...
> 各タブの役割（最新、1つ前...）が固定されているため、「リストの一番上が常に最新」と直感的に把握でき、複数オブジェクトのプロパティを安定して見比べることができます。
> Inspectorウィンドウ自体の番号（#1, #2...）は固定されており、どのウィンドウが「最新」の役割を担うかは、Inspector Manager上の並び順で決まります。
...
各行の要素：
- **ドラッグハンドル（≡）**: ドラッグ&ドロップでローテーション順序（役割）を並び替え
- **ロックアイコン**: クリックでロック/アンロック切り替え（後述）
- **Inspector番号**: ウィンドウの識別番号（固定）
- **ローテーションバッジ**: 現在の更新対象（▶）や順序を表示
...
> **想定ユースケース**: 履歴モードでは「リストの1番目 = 最新」なので、画面上で最も見やすい位置にあるInspector（例えば #1）をリストの一番上にドラッグすると快適です。

### 除外管理

ローテーション有効時、各行の「－」ボタンでそのInspectorをローテーション対象から除外できます。除外されたInspectorは「── 除外中 ──」セクションに移動し、ロック状態が維持されます。「＋」ボタンでローテーションに戻せます。

> **想定ユースケース**: 特定のオブジェクト（マテリアルやプレハブなど）を1つのInspectorに固定表示し、残りのInspectorだけでローテーションしたい場合に使います。

### ロック解除によるローテーション離脱

ローテーションは対象のInspectorを常にロック状態に保ちます。ロックアイコンやオーバーレイのボタンで**手動アンロックすると、そのInspectorはローテーションから外れ**、「── ローテーション対象外（選択に追従）──」セクションに移動します。この状態のInspectorは通常どおり選択に追従します。

再びロックすると、ローテーションの末尾に復帰します。

> **注意**: ローテーションから外すには、Inspector Managerのロックアイコン・オーバーレイのボタン・ショートカットのいずれかを使ってください。Inspector自身のタイトルバーにある南京錠アイコンで解除した場合は、ローテーションの一部として自動的にロックし直されます。ローテーション中のInspectorは常にロックされている必要があり、拡張はそこから外れた状態を「不整合」として復旧するためです。

> **想定ユースケース**: 1つのInspectorだけ「常に現在の選択を表示する」通常の挙動に戻したい場合に使います。除外（－ボタン）が「ロックしたまま固定表示」なのに対し、こちらは「選択に追従」になります。

### ロック状態の復元

ローテーションをOFFにすると、ローテーション開始前のロック状態に戻ります。Inspector Managerのウィンドウを閉じた場合も同様です。ローテーション中に手動でロックしていたInspectorが、まとめて解除されてしまうことはありません。
...
- **更新時にInspectorをフォーカス**: 更新されたInspectorを自動的に前面に表示します。HierarchyやProjectウィンドウから選択した場合は、操作の邪魔になるためフォーカスを移動しません。
